...
date: 2015-12-13T21:38:40.000Z
category: tools
tags:
  - containers
  - docker
excerpt: 'Docker: An Overview of Containerization Introduction There are often many roadblocks that stand in the way of easily moving your application through the development cycle and eventually into…'
wpCategory: docker
wpUrl: /docker/the-docker-ecosystem-an-overview-of-containerization/
cover: ../../assets/blog/containerTransperant2.png.jpg
coverAlt: 'The Docker Ecosystem: An Overview of Containerization'
---
...
![Docker containerization overview diagram — one Linux host kernel, the Docker engine, and three isolated containers (nginx, node app, postgres) each composed of read-only image layers and a writable container layer](../../assets/blog/docker-ecosystem-containerization-overview-2016.svg)
...
![Containers vs VMs comparison diagram — VMs (left) each carry a full guest OS on a hypervisor with 30-120s boot times; containers (right) share the host kernel via Docker engine and boot in under a second. Isolation in containers is process-level via cgroups + namespaces.](../../assets/blog/containers-vs-vms-resource-comparison-2016.svg)
...
![Docker image lifecycle diagram — source code + Dockerfile → docker build produces a layered image → docker push uploads to a registry (Docker Hub, ECR, GCR, ACR, or private) → production hosts docker pull and docker run to start containers from the same image](../../assets/blog/docker-image-lifecycle-build-push-pull-run-2016.svg)
