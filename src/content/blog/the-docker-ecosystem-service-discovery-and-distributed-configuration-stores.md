---
title: 'Docker: Service Discovery and Distributed Configuration Stores'
date: 2016-03-13T21:38:03.000Z
category: tools
tags:
...
![Service discovery diagram — services register their addresses to a distributed key/value store like etcd or Consul on startup; consumer services query the store at request time instead of hard-coding addresses](../../assets/blog/docker-ecosystem-service-discovery-2016.svg)
...
![DNS-based service discovery diagram — consumer queries cluster DNS for `api.local`; DNS returns the IP of a healthy container from the pool; the cluster manager (Consul, CoreDNS, Kubernetes kube-dns) keeps records in sync as containers come and go. Removed unhealthy entries don't appear in resolution.](../../assets/blog/docker-service-discovery-dns-pattern-2016.svg)
...
![Distributed config rollout diagram — admin (or CI) writes a config key to the kv store; every service instance is watching that key; the change propagates to all instances in seconds, no restarts needed. Versioned keys give an audit trail and easy rollback.](../../assets/blog/docker-service-discovery-config-rollout-2016.svg)
...
In the [next guide](https://www.digitalocean.com/community/tutorials/the-docker-ecosystem-networking-and-communication), we will discuss ways that Docker containers and hosts can communicate with customized networking configurations.

From <[https://www.digitalocean.com/community/tutorials/the-docker-ecosystem-service-discovery-and-distributed-configuration-stores](https://www.digitalocean.com/community/tutorials/the-docker-ecosystem-service-discovery-and-distributed-configuration-stores)>** 
